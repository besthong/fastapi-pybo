...
def question_list(db:Session = Depends(get_db), page:int = 0, size:int = 10, keyword: str=''):
    ...
    # db.close() #세션을 종료하는게 아닌, 커넥션 풀을 반환하는 함수이다.
...
@router.post("/create", status_code = status.HTTP_204_NO_CONTENT) #response_model 의 사용 대신, status_code를 사용하여 리턴응답없는경우 204 리턴
...
def question_update(_question_update: question_schema.QuestionUpdate,
    ...
    if not db_question.user:
        raise HTTPException(status_code = status.HTTP_400_BAD_REQUEST,
                            detail="질문에 사용자 정보가 없습니다.")
